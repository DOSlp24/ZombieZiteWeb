 ...
 */
@Singleton
class HomeController @Inject() (val messagesApi: MessagesApi, silhouette: Silhouette[DefaultEnv])(implicit webJarAssets: WebJarAssets, system: ActorSystem, mat: Materializer) extends play.api.mvc.Controller with I18nSupport {
  ...
  def about() = silhouette.SecuredAction.async { implicit request =>
    c.init(4)
  ...
  def showPlayground() = silhouette.SecuredAction.async { implicit request =>
    Future.successful(Ok(views.html.ZombieZite(c)))
  }

  def dead() = silhouette.SecuredAction.async { implicit request =>
    Future.successful(Ok(views.html.dead()))
  }

  def win() = silhouette.SecuredAction.async { implicit request =>
    Future.successful(Ok(views.html.win()))
  }

  def newGame(num: String) = silhouette.SecuredAction.async { implicit request =>
    c.init(Integer.parseInt(num))
    Future.successful(Ok(views.html.ZombieZite(c)))
  }

  def callWait() = silhouette.SecuredAction.async { implicit request =>
    c.wait(c.actualPlayer)
    Future.successful(Ok(views.html.ZombieZite(c)))
  }

  def callSearch() = silhouette.SecuredAction.async { implicit request =>
    c.search(c.actualPlayer)
    Future.successful(Ok(views.html.ZombieZite(c)))
  }

  def polymer() = silhouette.SecuredAction.async { implicit request =>
    Future.successful(Ok(views.html.Polymer()))
  }

  def callMove(direction: String) = silhouette.SecuredAction.async { implicit request =>
    direction match {
  ...
  def attackField(coord: String) = silhouette.SecuredAction.async { implicit request =>
  ...
  def equipArmor(invPosition: String) = silhouette.SecuredAction.async { implicit request =>
    val itemIndex = Integer.parseInt(invPosition)
    val item: ArmorInterface = c.actualPlayer.equipment.apply(itemIndex).asInstanceOf[ArmorInterface]
    c.equipArmor(c.actualPlayer, item)
    Future.successful(Ok(views.html.ZombieZite(c)))
  ...
  def equipWeapon(invPosition: String) = silhouette.SecuredAction.async { implicit request =>
    val itemIndex = Integer.parseInt(invPosition)
    val item: WeaponInterface = c.actualPlayer.equipment.apply(itemIndex).asInstanceOf[WeaponInterface]
    c.beweapon(c.actualPlayer, item)
    Future.successful(Ok(views.html.ZombieZite(c)))
  ...
  def unequipWeapon() = silhouette.SecuredAction.async { implicit request =>
    c.beweapon(c.actualPlayer, null)
    Future.successful(Ok(views.html.ZombieZite(c)))
  }

  def trashItem(invPosition: String) = silhouette.SecuredAction.async { implicit request =>
    val itemIndex = Integer.parseInt(invPosition)
    val item: Item = c.actualPlayer.equipment.apply(itemIndex)
    c.drop(c.actualPlayer, item)
    Future.successful(Ok(views.html.ZombieZite(c)))
